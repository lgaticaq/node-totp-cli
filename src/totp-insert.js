    ...
    .then(answers => {
      if (answers.code1 === answers.code2) {
        ...
        pass.stderr.on('data', data => {
          console.log(chalk.red(`Error: ${data}`)) // eslint-disable-line
        })
        pass.on('close', () => {
          console.log(chalk.green('Stored code')) // eslint-disable-line
          process.exit(0)
        })
      } else {
        console.log(chalk.red('Wrong code')) // eslint-disable-line
      }
    })
}
